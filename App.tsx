...
import { 
  ...
  Lock, CheckCircle, Link as LinkIcon, 
      ...
      <footer className={`border-t py-12 md:py-20 transition-colors ${isDark ? 'border-emerald-500/10 bg-black/40' : 'border-zinc-200 bg-white'}`}>
        <div className="max-w-6xl mx-auto px-6 text-center">
          ...
          <div className={`pt-8 border-t flex flex-col md:flex-row items-center justify-between gap-8 ${isDark ? 'border-white/5' : 'border-zinc-100'}`}>
            <p className={`text-[10px] font-bold uppercase tracking-widest ${isDark ? 'opacity-20' : 'text-zinc-300'}`}>
            ...
            <div className={`flex flex-wrap justify-center md:justify-end gap-6 md:gap-10 text-[10px] font-black uppercase tracking-widest ${isDark ? 'opacity-20' : 'text-zinc-300'}`}>
               <button onClick={() => setView('home')} className="hover:text-emerald-500 transition-colors">المحول</button>
               <button onClick={() => setView('policies')} className="hover:text-emerald-500 transition-colors">الخصوصية</button>
               <button onClick={() => setView(isAuthenticated ? 'admin' : 'login')} className="hover:text-emerald-500 transition-colors">الإدارة</button>
            </div>
          </div>
        </div>
      </footer>
