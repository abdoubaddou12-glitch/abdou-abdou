...
export default function App() {
  ...
  const [isDark, setIsDark] = useState(() => localStorage.getItem('theme_mode_v3') !== 'light');
  ...
  const [posts, setPosts] = useState<Post[]>(() => {
    const saved = localStorage.getItem('blog_posts_v3');
  ...
  const [adsterraConfig, setAdsterraConfig] = useState<AdsterraConfig>(() => {
    const saved = localStorage.getItem('adsterra_config_v3');
    return saved ? JSON.parse(saved) : { 
      isEnabled: true, 
      socialBar: '', 
      popUnder: '', 
  ...
  useEffect(() => {
    ...
    localStorage.setItem('theme_mode_v3', isDark ? 'dark' : 'light');
    localStorage.setItem('blog_posts_v3', JSON.stringify(posts));
    localStorage.setItem('total_converted', totalConverted.toString());
    localStorage.setItem('base_visitors', baseVisitors.toString());
    localStorage.setItem('adsterra_config_v3', JSON.stringify(adsterraConfig));
    
    if (!sessionStorage.getItem('v_tracked_v3')) {
        setTotalVisitors(prev => prev + 1);
        sessionStorage.setItem('v_tracked_v3', 'true');
    }
  ...
  return (
    <div className={`min-h-screen flex flex-col transition-colors duration-400 ${isDark ? 'text-white' : 'text-zinc-900'}`}>
      ...
      {/* Footer مع الإعلانات الحصرية في الأسفل */}
