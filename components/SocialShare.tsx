...
export const SocialShare: React.FC<SocialShareProps> = ({ isDark }) => {
  ...
  const shareText = "استكشف Storehalal - مدونتك التقنية وأداة تحويل الصور الاحترافية! 🚀";
  ...
  return (
    <div className="mt-12 w-full max-w-4xl mx-auto px-4 animate-slide-up">
        ...
        <div className="grid grid-cols-2 md:flex md:flex-wrap justify-center gap-2 md:gap-3 w-full">
          {socialLinks.map((social) => (
            <a 
              ...
              className={`flex items-center justify-center gap-2 py-4 rounded-2xl text-white font-bold text-[11px] md:text-xs transition-all active:scale-95 shadow-lg px-2 md:px-6 ${social.color}`}
            >
              {social.icon}
          ...
          <button 
            ...
            className={`col-span-2 md:col-auto flex items-center justify-center gap-2 px-6 py-4 rounded-2xl font-bold text-[11px] md:text-xs transition-all border shadow-sm ${
              isDark 
                ? 'bg-zinc-900 border-zinc-800 text-emerald-400 hover:border-emerald-500' 
                : 'bg-white border-zinc-200 text-zinc-700 hover:border-emerald-500'
            }`}
          >
